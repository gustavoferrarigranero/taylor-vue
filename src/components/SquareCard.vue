...
<script>
  export default {
    ...
    methods: {
      getCards() {
        this.$http.get('http://127.0.0.1:8080/squares/' + this.$route.params.id + '/cards').then((ret) => {
          if (ret.body) {
            this.cards = ret.body;
          }
        });
      ...
      getSquare() {
        this.$http.get('http://127.0.0.1:8080/squares/' + this.$route.params.id).then((ret) => {
          if (ret.body) {
      ...
      saveNewCard(){
        this.$http.post('http://127.0.0.1:8080/squares/' + this.$route.params.id + '/cards', this.card).then((ret) => {
          if (ret.body) {
      ...
      removeCard(card){
        this.$http.delete('http://127.0.0.1:8080/cards/' + card._id).then((ret) => {
          if (ret.body) {
      ...
      editCard(card){
        this.$http.put('http://127.0.0.1:8080/cards/' + card._id, card).then((ret) => {
          if (ret.body) {
            this.cards = this.cards.filter(function (card) {
